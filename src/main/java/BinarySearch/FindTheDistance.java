...
public class FindTheDistance {
    ...
    public int findTheDistanceValue(int[] arr1, int[] arr2, int d) {
        ...
        int count =0;
        for (int num :arr1) {
            int pos = binarySearch(arr2,num);
            boolean ok = true;
            if(pos<arr2.length){
                ok &= arr2[pos]-num>d;
            }
            if(pos-1>=0 && pos-1<=arr2.length){
                ok &= num-arr2[pos-1]>d;
            }
            count += ok ? 1:0;
        }
    ...
    public static void main(String[] args) {
        FindTheDistance findTheDistance = new FindTheDistance();
        int[] arr1 = new int[]{1,4,2,3};
        int[] arr2 = new int[]{-4,-3,6,10,20,30};
        System.out.println(findTheDistance.findTheDistanceValue(arr1,arr2,3));
    }
}
